...
- `main.py` — FastAPI backend. Talks to Twilio. Exposes:
  ...
  - `POST /api/send` — send a message to a list of recipients via Twilio. **Requires a valid session token.**
...
You run both processes at the same time.
...
6. Run the frontend (in a second terminal, with the venv activated):
   ```
   streamlit run app.py
   ```

7. Streamlit opens a browser tab (default `http://localhost:8501`).
   ...
   - **Send Message tab**: pick recipients/groups, write a message, and send.
...
This skips the manual venv/uvicorn/streamlit steps above (steps 3, 5, 6) — everything runs in two containers instead.

1. Complete steps 1, 2, and 4 above first (you still need a real `twilio.env` with `TOTP_SECRET` set, and a real `contacts.json` — both stay on your host machine, never baked into the image).
...
- `twilio.env` is passed in via `env_file`, not copied into the image — secrets never end up baked into a Docker layer.
...
The same `client.messages.create(...)` call in `main.py` (`/api/send` route) is the only piece that actually talks to Twilio. To build the failure/outage alerting idea on top of this:
